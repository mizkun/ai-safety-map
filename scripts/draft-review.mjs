...
import { reviewUnits, reviewFields, readerFields, reviewIssues } from '../lib/review-fingerprints.mjs';
...
const record = { id, version: 2, date: id.slice(0, 10), reviewer: '', decision: 'pending', assessments: { draft: { ...Object.fromEntries([...reviewFields, ...readerFields].map((f) => [f, ''])), primary: [{ source: '', locator: '', support: '' }] } }, entries: Object.fromEntries(stale.map((key) => [key, { hash: units[key], assessment: 'draft' }])) };
fs.mkdirSync(folder, { recursive: true });
fs.writeFileSync(folder + '/' + id + '.json', JSON.stringify(record, null, 2) + '\n', { flag: 'wx' });
console.log('Draft created for ' + stale.length + ' changed/dependent units. Review the reasoning, reader context, why each topic appears, and primary sources before setting decision to reviewed.');
