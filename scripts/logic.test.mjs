...
function change(fn) {
  ...
  fn(data);
  return logicIssues(data).join('\n');
}
test('the reviewed causal structure has no mechanical contradictions', () =>
  assert.deepEqual(logicIssues(original), []));
test('AND inputs cannot silently become OR in a drill-down', () => {
  assert.match(
    change((d) => {
      d.graphs['income-conditions'].mode = 'any';
    }),
    /contradicts/,
  );
  assert.match(
    change((d) => {
      d.edges['F1-F3'].requires.pop();
    }),
    /omits|reviewed AND/,
  );
});
...
test('self-improvement feedback must be marked explicitly', () =>
  assert.match(
    change((d) => {
      d.edges['R3-R2'].relation = 'conditional';
    }),
    /unmarked causal cycle/,
  ));
test('capability branches cannot make RSI or ASI mandatory', () => {
  assert.match(
    change((d) => {
      d.graphs['development-paths'].mode = 'all';
    }),
    /OR alternatives/,
  );
  assert.match(
    change((d) => {
      d.edges['ASI-C3'].relation = 'conditional';
    }),
    /oversight challenge/,
  );
  assert.match(
    change((d) => {
      d.edges['R0-C2a'].requires = ['R0', 'ASI'];
    }),
    /ASI bypass/,
  );
});
test('research factors cannot silently become peer scenarios or mandatory control inputs', () => {
  assert.match(
    change((d) => {
      delete d.routes.find((r) => r.id === 'acceleration').role;
    }),
    /optional factor/,
  );
  assert.match(
    change((d) => {
      d.routes.find((r) => r.id === 'acceleration').contexts = ['missing'];
    }),
    /valid scenario contexts/,
  );
  assert.match(
    change((d) => {
      d.edges['C3-L'].requires.push('R3');
    }),
    /reviewed AND/,
  );
});
test('financial crisis is not money obsolescence or extinction', () => {
  for (const to of ['P3', 'X'])
    assert.match(
      change((d) => {
        d.edges.shortcut = {
          ...d.edges['R1-R2'],
      ...
      [
        {
          ...
          date: '2026-09-10',
          entries: {
            'node:M2c1': { hash: before['node:M2c1'], assessment: 'missing' },
          },
        },
      ],
      original.sources,
...
test('removing an item requires a fresh inventory review', () => {
  const before = reviewUnits(original),
    changed = structuredClone(original);
  delete changed.news['unused'];
  changed.news.pop();
  assert.notEqual(
    before['inventory:current'],
    reviewUnits(changed)['inventory:current'],
  );
});
test('a generated draft cannot count as an approved review', () =>
  assert.match(
    reviewIssues(
      { 'node:C1': 'abc' },
      [
...
test('observed claims need a direct study and resolvable primary evidence', () => {
  assert.match(
    change((d) => {
      d.nodes.W5.status = 'observed';
      d.nodes.W5.research = ['distribution'];
    }),
    /no direct observational/,
  );
  assert.match(
    change((d) => {
      delete d.sources['astra-card'];
    }),
    /unknown primary evidence/,
  );
});
