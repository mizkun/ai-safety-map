...
- Turn on **Show current evidence** in the upper-right corner to color the map by evidence. The adjacent information button explains the colors and highlights a condition to examine in each scenario. Red means cases have been reported. Deeper yellow means a phenomenon has been shown in limited settings; pale yellow means related evidence exists, but the condition itself has not been established. Gray marks unconfirmed conditions. Each yellow card's evidence details explain the classification. A separate green marker identifies a tested mitigation, with its scope and limitations. These are evidence labels, not danger scores or scenario completion percentages.
...
Each node and connection records a review date and interval. Publication date, observation period, and review date remain separate. An old evaluation is not treated as a current capability ceiling. The **Current evidence and what to watch** questions are editorial selections, not proven unique bottlenecks.
