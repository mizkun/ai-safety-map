...
| フィールド | 意味 |
| --- | --- |
| id / title / shortTitle | 固定ID、詳細の見出し、カード用の短い表示名（全て必須） |
...
`lib/expanded-tree-layout.ts` で固定配置し、`lib/horizontal-tree-layout.ts` で左から右へ変換します。横位置は年月ではありません。研究の加速の影響線、分類上の案内、現象の因果線は異なる意味です。`TreeExpansion` は表示中の経路と展開した親ノードのIDを持つ表示状態で、原データの条件や接続を変更しません。

## ストーリーと補助資料

`stories.json` は経路IDごとのtitle、intro、chapters、outlookを持ちます。各chapterはtitle、textと、関連するnodesの配列を持ちます。物語は仮説の筋道を説明し、未確認の出来事や確定した時期を作りません。ツアーはこの章と関連ノードから案内を組み立て、別の因果データを持ちません。説明の順番を条件の発生順とせず、章の関連項目をまとめて強調します。
...
初期HTMLには地図の構造・名称・用語の別名などだけを含めます。本文や研究カードは、内容の指紋を名前に含む `content/details-ja-….json` または `details-en-….json` から、選択中の言語で最初に詳細やツアーを開いた時に取得します。一度取得すると、同じページ内の次の詳細で再利用します。完全版はSSRの初期データへ重複埋め込みしません。
