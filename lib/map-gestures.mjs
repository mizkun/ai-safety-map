...
export function initialMapScale(viewport, layout) {
  if (layout.entryView === 'phone-overview')
    return Math.min(1, Math.max(0.9, viewport.height / layout.height));
  const fit = Math.min(
    1,
...
export function initialMapCamera(viewport, layout, anchor) {
  if (layout.entryView === 'phone-overview')
    return { scale: initialMapScale(viewport, layout), left: 0, top: 0 };
  ...
  if (layout.flow === 'vertical')
    return {
      ...
      left: Math.max(
        0,
        (anchor.x + anchor.width / 2) * scale - viewport.width / 2,
      ),
      top: Math.max(0, anchor.y * scale - 20),
    };
