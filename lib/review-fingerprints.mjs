...
export function reviewUnits(data, ui = null) {
  ...
  for (const [id, value] of Object.entries(data.current?.routes || {})) add('current', id, { value, sources: refs(value.sources, 'source'), nodes: refs([value.node, value.observed], 'node') });
  for (const [id, value] of Object.entries(data.current?.safeguards || {})) add('safeguard', id, { value, sources: refs(value.sources, 'source'), research: refs(value.research, 'research'), nodes: refs([id], 'node') });
  for (const [id, value] of Object.entries(data.current?.evidence || {})) add('evidence', id, { value, research: refs(value.research, 'research'), nodes: refs([id], 'node') });
