...
export function logicIssues(data) {
  ...
  for (const route of data.routes) {
    if (route.role && route.role !== 'factor') fail(route.id, 'unknown route role');
    if (route.role === 'factor' && (!route.contexts?.length || route.contexts.some((id) => !data.routes.some((r) => r.id === id && r.role !== 'factor')))) fail(route.id, 'a factor needs valid scenario contexts');
  }
  if (data.routes.find((r) => r.id === 'acceleration')?.role !== 'factor') fail('acceleration', 'research acceleration must remain an optional factor, not a peer scenario');
